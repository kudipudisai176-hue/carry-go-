...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // All users are now unified sender_receiver
      ...
      const res = await signup({
        ...
        role: finalRole as any, // Using any to bypass legacy role checks if they exist
        dob,
        gender,
        idProofType: idType,
        idNumber,
      ...
      if (res.success) {
        ...
        toast.error(res.message || "Signup failed. Please check your details.");
      }
    } catch (err: any) {
      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-50 px-4">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center rounded-3xl bg-white p-12 text-center shadow-2xl border border-white/60">
           ...
           <div className="h-2 w-32 bg-muted rounded-full overflow-hidden">
             <motion.div initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 2.0 }} className="h-full bg-green-500" />
           </div>
        </motion.div>
      </div>
    );
  ...
  const renderStepTwo = () => (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
       <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input type="date" value={dob} onChange={(e) => setDob(e.target.value)} className="h-12 pl-11 rounded-xl bg-slate-50 border-slate-200 font-medium" />
            </div>
          </div>
          <div className="space-y-2">
            <Label className="font-semibold text-slate-600">Gender</Label>
            <select value={gender} onChange={(e) => setGender(e.target.value)} className="w-full h-12 px-4 rounded-xl bg-slate-50 border-slate-200 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500">
       ...
       <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-2">
                ...
                <select value={idType} onChange={(e) => setIdType(e.target.value)} className="w-full h-12 px-4 rounded-xl bg-slate-50 border-slate-200 font-medium focus:outline-none focus:ring-2 focus:ring-purple-500">
                   {idTypes.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
             </div>
             <div className="space-y-2">
                <Label className="font-semibold text-slate-600">{idType} Number</Label>
                <Input value={idNumber} onChange={(e) => setIdNumber(e.target.value)} placeholder={`Enter ${idType} number`} className="h-12 rounded-xl bg-slate-50 border-slate-200 font-medium" />
             </div>
          </div>
          ...
          <div className="space-y-2">
             ...
             <div className="flex items-center gap-4">
                <div className="relative flex-1">
                   ...
                   <Input type="file" accept="image/*" onChange={(e) => handleFileChange(e, setIdPhoto)} className="h-12 pl-11 rounded-xl bg-slate-50 border-slate-200 file:hidden cursor-pointer" />
                </div>
       ...
       <div className="flex gap-3">
          <Button variant="outline" onClick={handlePrev} className="h-14 rounded-2xl border-slate-200 font-bold px-6">Back</Button>
  ...
  const renderStepThree = () => (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
       ...
       <div className="flex flex-col items-center gap-6">
          <div className="relative h-48 w-48 rounded-[2.5rem] bg-slate-100 border-4 border-dashed border-slate-200 flex items-center justify-center overflow-hidden">
             {livePhoto ? (
               <img src={livePhoto} className="h-full w-full object-cover" />
             ) : (
               <User className="h-16 w-16 text-slate-300" />
             )}
             <Button onClick={() => setIsCameraOpen(true)} className="absolute bottom-2 h-10 w-10 rounded-full bg-white text-secondary shadow-lg p-0 hover:scale-110">
                <Camera className="h-5 w-5" />
             </Button>
          </div>

          <div className="w-full space-y-4 pt-4">
             <div className="flex items-center space-x-3 rounded-2xl border border-slate-100 bg-slate-50/50 p-4">
                <Checkbox id="terms" checked={agreedTerms} onCheckedChange={(checked) => setAgreedTerms(checked as boolean)} className="h-5 w-5 rounded-md border-slate-300" />
             ...
             <div className="flex gap-3">
                <Button variant="outline" onClick={handlePrev} className="h-14 rounded-2xl border-slate-200 font-bold px-6">Back</Button>
                <Button onClick={handleSubmit} disabled={submitting} className="flex-1 h-14 rounded-2xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-black text-lg shadow-xl shadow-orange-500/30">
