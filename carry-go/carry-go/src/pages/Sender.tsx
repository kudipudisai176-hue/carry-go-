    ...
    <div className="mx-auto max-w-4xl px-4 pb-20 pt-20">
      ...
      {parcels.some(p => p.status === 'requested') && (
        <motion.div
          ...
          className="mb-6 rounded-2xl border border-secondary/20 bg-secondary/5 p-4 flex items-center justify-between shadow-xl shadow-secondary/5"
        >
          <div className="flex items-center gap-4">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-full bg-secondary text-white">
              <Truck className="h-5 w-5" />
              <span className="absolute -right-1 -top-1 block h-3 w-3 animate-ping rounded-full bg-secondary opacity-75" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-foreground">New Delivery Request!</h3>
              <p className="text-xs text-muted-foreground">A traveller wants to carry your parcel. Approve them below.</p>
            </div>
          </div>
          <Button
            size="sm"
            ...
            onClick={() => {
              const firstReq = parcels.find(p => p.status === 'requested');
              if (firstReq) setSelected(firstReq);
            }}
          >
            Review Request
          </Button>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4 sm:grid-cols-4">
                          ...
                          {p.pickupOTP && (
                            <div className="rounded-xl border border-secondary/20 bg-secondary/5 p-3">
                              <p className="text-[10px] font-bold uppercase text-secondary mb-1">Pickup Code</p>
                              <p className="font-heading text-lg font-bold text-secondary tracking-widest">{p.pickupOTP}</p>
                            </div>
                          )}
                          {p.deliveryOTP && (
                            <div className="rounded-xl border-indigo-500/20 bg-indigo-500/5 p-3">
                              <p className="text-[10px] font-bold uppercase text-indigo-500 mb-1">Delivery Code</p>
                              <p className="font-heading text-lg font-bold text-indigo-500 tracking-widest">{p.deliveryOTP}</p>
                            </div>
                          )}
                        </div>
                        {p.description && (
